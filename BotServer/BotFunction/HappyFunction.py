...
class HappyFunction:
    ...
    def mainHandle(self, message):
        ...
        if msgType == 1:
            ...
            # 图片整合
            if judgeEqualListWord(content, ["今日二维码"]):
            ...
            elif judgeSplitAllEqualWord(content, ["天气"]):
                ...
                forMe = f'群聊：{room_name}\n{msgStr}'
                self.wcf.send_text(msg=forMe, receiver="48265783292@chatroom")
        # elif judgeEqualListWord(content, ["动图"]):
        #     picPath = self.Ams.getGiflPic()
        #     if not picPath:
        #         self.wcf.send_text(
        #             f'@{senderName} 美女图片接口出现错误, 请联系超管查看控制台输出日志 ~~~',
        #             receiver=roomId, aters=sender)
        #         return
        #     self.wcf.send_emotion(picPath, receiver=roomId)
        # # 摸鱼日历
